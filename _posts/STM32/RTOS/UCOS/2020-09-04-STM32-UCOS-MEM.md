...
description: UCOS内存管理
---
# 内存管理常用函数
- 创建内存管理对象`void OSMemCreate(OS_MEM *p_mem,CPU_CHAR *p_name,void *p_addr,OS_MEM_QTY _blks,OS_MEM_SIZE blk_size,OS_ERR p_err)`
- 内存申请`void *OSMemGet(OS_MEM *p_mem,OS_ERR p_err)`
- 内存释放`void OSMemPut(OS_MEM *p_mem,void *p_blk,OS_ERR *p_err)`
...
static  void  AppTaskPost ( void * p_arg )
{
    ...
    while (DEF_TRUE) {                                                         //任务体
        ...
        OSTimeDlyHMSM ( 0, 0, 1, 0, OS_OPT_TIME_DLY, & err );        //每1s发送一次                 //每20ms扫描一次
    }
}
/**
 * @brief  Pend任务主体
